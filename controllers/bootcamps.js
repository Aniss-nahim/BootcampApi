...
exports.updateBootcamp = asyncHandler(async (req, res, next) => {
  ...
  if (bootcamp.user.toString() !== req.user.id && req.user.role !== "admin") {
    return next(
      ErrorApi.Forbidden(
        `User with ID ${req.user.id} is unauthorized to update this bootcamp`
      )
    );
  }
...
exports.getBootcampInRadius = asyncHandler(async (req, res, next) => {
  ...
  const bootcamps = await Bootcamp.find({
    location: {
      $geoWithin: {
        $centerSphere: [
          [centerLocation[0].longitude, centerLocation[0].latitude],
          radius,
        ],
      },
    },
...
exports.uploadBootcampPhoto = asyncHandler(async (req, res, next) => {
  ...
  if (file.size > process.env.IMAGE_MAX_SIZE) {
    ...
    return next(
      ErrorApi.BadRequest(
        `Please upload an image with maximum size of ${maxSize}Mo`
      )
    );
  }
